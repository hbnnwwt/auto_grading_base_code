...
本项目实现一套基于计算机视觉的答题卡自动评分系统。系统从图像预处理出发，依次完成学号识别、选择题/判断题填涂识别、简答题 OCR，最后进行答案比对与评分。

**课程定位：** 计算机视觉（微）课程设计项目，通过实现一个完整的智能阅卷系统，综合运用图像预处理、形态学操作、轮廓检测、OCR 等核心知识点。

**特点：** 8 个核心模块提供骨架模板（`raise NotImplementedError`），学生需根据启发式提问独立实现算法逻辑，训练解决实际问题的能力。
...
| 类别 | 技术 | 说明 |
|------|------|------|
| **图像处理** | OpenCV | 灰度化、二值化、形态学、轮廓检测 |
| **OCR** | PaddleOCR / EasyOCR / RapidOCR | 手写文字识别 |
| **GUI** | Streamlit | 交互式 Web 界面 |
| **评分** | LLM API（可选） | 语义评分简答题 |
| **测试** | pytest | 单元测试框架 |
| **配置** | JSON + Excel | 布局参数、参考答案管理 |
...
│   └── grading.py                # 评分逻辑
│   # ===== 以下模块为扩展/加分项（可选） =====
│   ├── llm_essay_grader.py    # LLM 语义评分（扩展阅读）
│   └── blank_calibrator.py     # 空白卷标定（扩展阅读）
├── answer_sheet_generator/   # 答题卡模板生成器（扩展阅读）
│   └── ...
...
│   └── ...
└── data/
    ├── answer_sheets/            # 示例答题卡图片（4张原始 + 6张测试集，共10张）
...
系统有 **8 个骨架模块**，每个模块对应一个独立算法挑战。每个骨架文件包含接口定义和**启发式提问**（只问不答，不给步骤），帮助你观察数据特征、思考算法原理后再动手实现。
...
**启发式问题：** 轮廓检测得到的边界框与网格行列之间有何关系？逐列填充率分析时，如何区分"填涂"与"空白"？
...
**启发式问题：** 连通域气泡定位后，如何将其映射到固定网格的行列坐标？填充率比较的阈值应如何选取？
...
**启发式问题：** 判断题只有 T/F 两个选项，填涂特征与选择题有何不同？如何复用已有逻辑并做针对性调整？
...
**启发式问题：** 不同 OCR 引擎返回的 bounding box 格式是否一致？为什么要按 Y 坐标排序而不是直接使用返回顺序？
...
**启发式问题：** 如何处理学生未作答的题目（识别结果为 None）？简答题与客观题的评分逻辑有何本质区别？
...
以下模块已有完整实现，不参与核心评分流程，供学有余力的同学深入学习：

| 模块 | 说明 |
|------|------|
| `llm_essay_grader.py` | 调用大语言 API 对简答题进行语义评分（非字符匹配） |
| `blank_calibrator.py` | 通过空白答题卡自动标定填涂阈值，提升识别鲁棒性 |
| `answer_sheet_generator/` | 根据配置动态生成可打印的 HTML 答题卡模板 |
