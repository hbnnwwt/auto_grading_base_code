...
def mark_wrong_on_page(page, region, cell_results, grading_detail,
    ...
    Args:
        ...
        cell_results: 单元格识别结果
        grading_detail: 评分结果字典 {题号: {'correct': ..., 'given': ..., 'score': ...}}
        option_labels: 选项标签列表，如 ['A','B','C','D'] 或 ['T','F']
        color: ×颜色
        thickness: ×线条粗细
    """
    ...
    for cell in cell_results:
        ...
        # 判断是否需要画×：错选(0分)、多选、未选
        is_wrong = score == 0  # 答错
        is_blank = (given is None or given == '')  # 未选
        is_multi = (given is not None and len(given) > 1)  # 多选
