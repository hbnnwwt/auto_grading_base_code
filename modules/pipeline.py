...
if os.path.exists(_LAYOUT_PATH):
    ...
    # 从 layout 模块复用 fallback 值，避免重复定义
    LAYOUT = {
        ...
        "judge": {"rows": 3, "cols": 4, "question_start": 21,
                  "question_count": 10},
        "layout": {
            "page1_fallback": {k: list(v) for k, v in LayoutAnalyzer.PAGE1_FALLBACK.items()},
            "page2_fallback": {k: list(v) for k, v in LayoutAnalyzer.PAGE2_FALLBACK.items()},
        },
    }
...
def extract_student_id(image, regions, digit_count=None, threshold=0.3):
    """从版面区域中提取学号。"""
...
def recognize_choices(image, regions, threshold=0.06, return_details=False,
                      blank_baseline=None, zone_bounds=None):
    """识别选择题答案。"""
    _ch = LAYOUT['choice']
    ...
    if return_details:
        return result['answers'], result['cell_results']
...
def process_student_pair(page1_path, page2_path=None,
                         ...
                         digit_count=None, threshold=0.5,
                         essay_questions=None,
                         choice_baseline=None, judge_baseline=None):
    """处理一个学生的答题卡（第一页 + 第二页）。

    Args:
        choice_baseline: 选择题空白基准 dict {q: [meanA, meanB, ...]}
        judge_baseline: 判断题空白基准 dict {q: [meanT, meanF]}
