...
from modules.blank_calibrator import (
    load_baseline, get_choice_baseline_dict, get_judge_baseline_dict,
    get_choice_zone_bounds, get_judge_zone_bounds,
)
from modules.defaults import (
    DEFAULT_BASE_URL, DEFAULT_LLM_MODEL, DEFAULT_OCR_MODEL,
    IMAGE_EXTS, path_constants, natural_sort_key,
)
...
def _build_essay_grader(args):
    ...
    api_keys_fallback = keys.get("api_keys", [])
    if not api_key and api_keys_fallback:
        api_key = api_keys_fallback
    elif api_key and api_keys_fallback:
        all_keys = [api_key]
        all_keys.extend(k for k in api_keys_fallback if k and k not in all_keys)
        api_key = all_keys[0] if len(all_keys) == 1 else all_keys

    # 合并主 base_url 和备用列表
    base_url = args.base_url or mcfg.get("base_url", DEFAULT_BASE_URL)
    fallback_urls = mcfg.get("fallback_base_urls", [])
    if fallback_urls:
        all_urls = [base_url] if base_url else []
        all_urls.extend(u for u in fallback_urls if u and u not in all_urls)
        base_url = all_urls[0] if len(all_urls) == 1 else all_urls

    # 合并主 model 和备用列表
    model = args.model or mcfg.get("llm_model", DEFAULT_LLM_MODEL)
    fallback_models = mcfg.get("llm_models", [])
    if fallback_models:
        all_models = [model] if model else []
        all_models.extend(m for m in fallback_models if m and m not in all_models)
        model = all_models[0] if len(all_models) == 1 else all_models

    has_key = bool(api_key) if isinstance(api_key, list) else bool(api_key.strip())
    if not has_key:
...
def batch_process(args, service, ocr_config):
    ...
    batch_output_dir = os.path.join(processed_dir, timestamp)
    ...
    for i in range(pair_count):
        ...
        if not args.no_mark and p1 is not None and p2 is not None:
            mark_and_save(student_id, p1, p2, r1, r2,
                          ...
                          essay_max=essay_max, output_dir=batch_output_dir)
            print(f"  标注图已保存到: {batch_output_dir}/")
...
def single_process(args, service, ocr_config):
    ...
    corrected, gray, _, binary = preprocessor.process(image)
    regions = analyzer.analyze(corrected, binary, page=args.page)
    ...
    if args.page == 1:
        ...
        recognized['choice'] = recognize_choices(
            corrected, regions, args.threshold,
            blank_baseline=_CHOICE_BASELINE,
            zone_bounds=_CHOICE_ZONE_BOUNDS)
        print(f"学号: {student_id}")
        print(f"选择题: {recognized['choice']}")
    else:
        recognized['judge'] = recognize_judges(
            corrected, regions, args.threshold,
            blank_baseline=_JUDGE_BASELINE,
            zone_bounds=_JUDGE_ZONE_BOUNDS)
