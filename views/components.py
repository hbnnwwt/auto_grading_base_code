...
def check_and_save_rejected(student_id, page1_orig, page2_orig,
                            uuid_str, processed_dir):
    """检测学号是否未填涂（废卷），若是则保存废卷图片并返回结果。

    判断标准：学号中 '?' 占比 ≥ 80% → 废卷。

    Returns:
        None 或 dict: 返回结果 dict（含 rejected=True），非废卷返回 None。
    """
    if not student_id:
        return None
    if student_id.count('?') < len(student_id) * 0.8:
        return None  # 正常学号

    os.makedirs(processed_dir, exist_ok=True)
    废卷_id = f"废卷_{uuid_str[:8]}"
    p1_path = os.path.join(processed_dir, f"{废卷_id}_正面.png")
    p2_path = os.path.join(processed_dir, f"{废卷_id}_背面.png")
    imwrite(p1_path, page1_orig)
    imwrite(p2_path, page2_orig)
