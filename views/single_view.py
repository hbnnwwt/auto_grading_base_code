...
def render_single(threshold, llm_enabled, llm_api_key, llm_base_url,
    ...
    # ── 单套模式 LLM 重试处理 ──
    if st.session_state.get("_single_llm_retry") and st.session_state.get("_single_recognized"):
        recognized = st.session_state["_single_recognized"]
        svc = None
        if os.path.exists(PATHS['answer_key']):
            try:
                svc = GradingService.from_xlsx(PATHS['answer_key'])
            except Exception as e:
                st.error(f"参考答案加载失败: {e}")
        ...
        if svc:
            try:
                ...
                # ── 重新生成标注图 ──
                p1_bytes = st.session_state.get("_single_p1_bytes")
                p2_bytes = st.session_state.get("_single_p2_bytes")
                if p1_bytes and p2_bytes:
                    try:
                        preprocessor = ImagePreprocessor()
                        analyzer = LayoutAnalyzer()
                        page1 = load_image_from_bytes(p1_bytes)
                        page2 = load_image_from_bytes(p2_bytes)
                        ...
                        _, _, marked_p1, marked_p2 = mark_and_save(
                            recognized.get("student_id") or "unknown",
                            page1, page2,
                            regions1, regions2,
                        ...
                        st.session_state["_single_marked_p1"] = marked_p1
                        st.session_state["_single_marked_p2"] = marked_p2
                    except Exception as e:
                        st.warning(f"标注图更新失败: {e}")
            except Exception as e:
                st.error(f"评分失败: {e}")
        st.session_state["_single_llm_retry"] = False
    ...
    if page1_file is not None and page2_file is not None:
        ...
        # 展示重试后的简答题结果
        if st.session_state.get("_single_result"):
            result = st.session_state["_single_result"]
            essay_detail = result.get("essay_detail", {})
            if essay_detail:
                st.subheader("简答题评分（重试后）")
        ...
        if st.button("开始识别", type="primary", width='stretch'):
            # 清除之前重试的缓存
            for key in ["_single_result", "_single_recognized", "_single_llm_retry",
                        "_single_p1_bytes", "_single_p2_bytes",
                        "_single_marked_p1", "_single_marked_p2"]:
                if key in st.session_state:
                    del st.session_state[key]
            try:
                p1_bytes = page1_file.read()
                p2_bytes = page2_file.read()
                st.session_state["_single_p1_bytes"] = p1_bytes
                st.session_state["_single_p2_bytes"] = p2_bytes
                page1_img = load_image_from_bytes(p1_bytes)
                page2_img = load_image_from_bytes(p2_bytes)
            except Exception as e:
                st.error(f"图片读取失败: {e}")
                st.stop()
            ...
            with st.expander("Step 4  OMR 识别（填涂检测）", expanded=True):
                ...
                if regions1.get('student_id'):
                    ...
                    sid_rec = StudentIdRecognizer(
                        digit_count=LAYOUT.get('student_id', {}).get('digit_count', 10),
                        threshold=threshold)
                    ...
                    # ── 废卷检测：学号未填涂 → 保存废卷图片后停止 ──
                    reject_result = check_and_save_rejected(
                        student_id, p1_orig, p2_orig,
                        uuid_str, PATHS['processed_dir'])
                    if reject_result:
                        st.warning(
                            f"⚠️  学号 `{student_id}` 未填涂或填涂无效，判定为废卷。"
                            "已保存废卷图片，跳过后续识别。")
                        st.info(f"已保存：{PATHS['processed_dir']}/废卷_{uuid_str[:8]}_正面.png")
                        st.info(f"已保存：{PATHS['processed_dir']}/废卷_{uuid_str[:8]}_背面.png")
                        st.stop()
                ...
                if regions1.get('choice'):
                    ...
                    choice_rec = ChoiceRecognizer(
                        threshold=threshold,
                        blank_baseline=choice_baseline,
                        zone_bounds_template=choice_zone_bounds)
                ...
                if regions2.get('judge'):
                    ...
                    judge_rec = JudgeRecognizer(
                        threshold=threshold,
                        blank_baseline=judge_baseline,
                        zone_bounds_template=judge_zone_bounds)
            ...
            with st.expander("Step 5  OCR 识别（简答题）", expanded=True):
                if regions2.get('essay'):
                    ...
                    with c2:
                        ...
                        _eq = get_essay_questions()
                        _q = _eq[0] if _eq else 31
                        st.markdown(f"**第{_q}题 OCR 结果**")
            ...
            with st.expander("Step 6  评分", expanded=True):
                ...
                st.session_state["_single_recognized"] = recognized
                try:
                    result = svc.grade(recognized)
                except Exception as e:
                    st.error(f"评分失败: {e}")
                ...
                if llm_enabled:
                    ...
                    if has_llm_failure and svc and svc.essay_grader:
                        if st.button("重试 LLM 评分", key="single_retry_llm"):
                            st.session_state["_single_llm_retry"] = True
                            st.rerun()
                else:
                ...
                # 优先使用 session_state 中的标注图（重试后可能已更新）
                display_p1 = st.session_state.get("_single_marked_p1", marked_p1)
                display_p2 = st.session_state.get("_single_marked_p2", marked_p2)
                st.divider()
                st.markdown("**批改标注图（红色 × = 错题）**")
                c1, c2 = st.columns(2)
                with c1:
                    st.caption("第1页（选择题）")
                    st.image(cv2.cvtColor(display_p1, cv2.COLOR_BGR2RGB),
                             width='stretch')
                with c2:
                    st.caption("第2页（判断题）")
                    st.image(cv2.cvtColor(display_p2, cv2.COLOR_BGR2RGB),
                             width='stretch')
